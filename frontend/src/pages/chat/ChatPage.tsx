...
const ChatPage = () => {
    ...
    return (
        <main className='h-full rounded-lg bg-gradient-to-b from-zinc-800 to-zinc-900 overflow-hidden'>
            <Topbar />
            <div className='grid lg:grid-cols-[.35fr_1fr] grid-cols-[80px_1fr] h-[calc(100vh-180px)]'>
                <UsersList />
                {/* chat message */}
                                ...
                                <div className='p-4 space-y-4'>
                                    {messages.map((message) => (
                                        <div key={message._id} className={`flex items-start gap-3 ${message.senderId === user?.id ? "flex-row-reverse" : ""}`}>
                                            ...
                                            <div className={`rounded-lg p-3 max-w-[70%] ${message.senderId === user?.id ? "bg-green-600" :"bg-zinc-800"}`}>
